...
//        return pimpl_->status.lower_level_error;
        return pimpl_->status.lower_level_error.isEmpty() || (pimpl_->status.top_block_height < pimpl_->status.top_known_block_height && pimpl_->status.lower_level_error == "SEND_ERROR") ? tr("Connected to infiniumd") : tr("Infiniumd status: %1").arg(pimpl_->status.lower_level_error);
