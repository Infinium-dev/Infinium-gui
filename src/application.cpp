...
const char VERSION_DATA_URL[] = "https://raw.githubusercontent.com/Infinium-dev/Infinium-gui/main/LatestInfiniumStableVersion.txt?1"; // use ?1 trick to force reload and bypass cache
...
{
    setApplicationName("infinium"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Infinium Wallet") + ' ' + Settings::getFullVersion());
...
bool WalletApplication::init()
{
    ...
    {
        WalletLogger::warning(tr("[Application] Infinium wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Infinium wallet GUI already running"));
        return false;
    }
...
{
    splashMsg(tr("Looks like this is your first run of the new Infinium Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote walletd daemon."));
}
