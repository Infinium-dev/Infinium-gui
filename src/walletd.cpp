...
void BuiltinWalletd::run()
{
    ...
    if (Settings::instance().getBytecoindConnectionMethod() != ConnectionMethod::BUILTIN)
        args << QString{"--infiniumd-remote-address=%1"}.arg(Settings::instance().getBytecoindEndPoint());
...
QString BuiltinWalletd::errorMessage(ReturnCodes err)
{
    ...
    case ReturnCodes::WALLET_FILE_EXISTS:
        msg = tr("The specified wallet file already exists. Infinium wallet never overwrites an existed wallet file for safety reason. If you want to overwrite the file please remove it manually and try again\n(") + Settings::instance().getWalletFile() + ").";
        break;
